...
/** Keep this function self-contained: install.ts embeds its source in the standalone OpenCode plugin. */
export function classifyQuotaSignal(payload: unknown): QuotaSignalKind | null {
  ...
  const properties = event !== null && typeof event === 'object'
    ? Reflect.get(event, 'properties')
    : undefined;
  const retryStatus = properties !== null && typeof properties === 'object'
    ? Reflect.get(properties, 'status')
    : undefined;
  const propertyError = properties !== null && typeof properties === 'object'
    ? Reflect.get(properties, 'error')
    : undefined;
  const eventError = event !== null && typeof event === 'object'
  ...
  const retryError = retryStatus !== null && typeof retryStatus === 'object'
    ? Reflect.get(retryStatus, 'error')
    : undefined;
  const structured = [event, error, errorData, retryStatus, retryError];
